...
This is an isolated test copy of `E:\POS Preview` for INET Cloud Windows Bridge UAT preparation.

Scope for this copy:

- Preserve the POS Preview monorepo structure.
- Test INET Cloud payment bridge integration planning only.
- Do not implement INET Cloud API changes yet.
- Do not store real secrets in this repository.
- Use `.env.inet-cloud.example` as the sanitized UAT placeholder template.
- See `docs/inet-cloud-uat-plan.md` for architecture, inspection targets, checklist, and security notes.
- See `docs/INET-CLOUD-CUTOVER-PLAN.md` for the GitHub + Vercel + Bridge + INET Cloud server/database migration path.

Run these commands from `E:\POS-INET-Cloud-UAT` after opening the copied project:
